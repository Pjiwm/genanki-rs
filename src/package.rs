...
impl Package {
    ...
    fn write_maybe_timestamp<W: Write + Seek>(
        ...
        outzip
            .start_file::<&str, ExtendedFileOptions>("collection.anki2", FileOptions::default())
            .map_err(zip_error)?;
        ...
        outzip
            .start_file::<&str, ExtendedFileOptions>("media", FileOptions::default())
            .map_err(zip_error)?;
        outzip.write_all(media_json.as_bytes())?;

        for (idx, &path) in &media_file_idx_to_path {
            outzip
                .start_file::<String, ExtendedFileOptions>(idx.to_string(), FileOptions::default())
                .map_err(zip_error)?;
